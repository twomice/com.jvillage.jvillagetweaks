...
class CRM_Contact_Form_Search_Custom_InHonorOf extends CRM_Contact_Form_Search_Custom_Base implements CRM_Contact_Form_Search_Interface {
    ...
    function __construct( &$formValues ) {
         ...
         */
        $this->_columns = array(  ts('Donor Sort Name')   => 'sort_name',
...
$result = civicrm_api('CustomField', 'getsingle', $params);
 ...
 CASE WHEN soft.id IS NOT NULL THEN con2.sort_name 
 ...
 CASE WHEN soft.id IS NOT NULL THEN con2.display_name 
 ...
 CASE WHEN soft.id IS NOT NULL THEN soft.amount ELSE contr.total_amount END as total_amount,
 ft.name as financial_type_name, 
 date(contr.receive_date) as contrib_date, 
 contrib_status.contrib_status_label, 
  CASE WHEN soft.id IS NOT NULL THEN soft_type.honor_type_label ELSE tribute_type.tribute_type_label END as honor_type_label,
  tribute.$t_note as tribute_note,  
  CASE WHEN soft.id IS NOT NULL THEN con2.sort_name ELSE  concat( ifnull(tribute.$t_n_last_name, ''), ', ' , ifnull(t_n_prefix.t_n_prefix_label, '') , ' ', ifnull(tribute.$t_n_first_name, '') ) END as t_n_sort_name,
  CASE WHEN soft.id IS NOT NULL THEN con2.display_name ELSE  concat( ifnull(t_n_prefix.t_n_prefix_label, ''), ' ',  ifnull(tribute.$t_n_first_name, '') , ' ' ,  ifnull(tribute.$t_n_last_name, '') ) END as t_n_display_name,
   CASE WHEN soft.id IS NOT NULL THEN s_email.email ELSE tribute.$t_n_email END as t_n_email, 
   ...
   CASE WHEN soft.id IS NOT NULL THEN s_addr.postal_code ELSE tribute.$t_n_postal_code END as t_n_postal_code   ";
        ...
        $select  = " contact_a.id AS contact_id, contact_a.sort_name AS sort_name, contact_a.display_name AS display_name,
 CASE WHEN contr.honor_type_id IS NOT NULL THEN con2.sort_name 
 ELSE concat( ifnull(tribute.$t_last_name, '') , ', ', ifnull(tribute_prefix.tribute_prefix_label, '') , ' ', ifnull(tribute.$t_first_name, '') )  END AS tribute_sort_name, 
 CASE WHEN contr.honor_type_id IS NOT NULL THEN con2.display_name 
 ELSE concat( ifnull(tribute_prefix.tribute_prefix_label, ''), ' ', ifnull(tribute.$t_first_name, '') , ' ',  ifnull(tribute.$t_last_name, '')    )  END AS tribute_display_name, 
 contr.total_amount as total_amount,  ft.name as financial_type_name, 
 date(contr.receive_date) as contrib_date, 
 contrib_status.contrib_status_label, 
  CASE WHEN contr.honor_type_id IS NOT NULL THEN honor_type.honor_type_label ELSE tribute_type.tribute_type_label END as honor_type_label,
  tribute.$t_note as tribute_note,  
  CASE WHEN contr.honor_type_id IS NOT NULL THEN con2.sort_name ELSE concat( ifnull(tribute.$t_n_last_name, ''), ', ' , ifnull(t_n_prefix.t_n_prefix_label, '') , ' ', ifnull(tribute.$t_n_first_name, '') ) END as t_n_sort_name,
  CASE WHEN contr.honor_type_id IS NOT NULL THEN con2.display_name ELSE concat( ifnull(t_n_prefix.t_n_prefix_label, ''), ' ',  ifnull(tribute.$t_n_first_name, '') , ' ' ,  ifnull(tribute.$t_n_last_name, '') ) END as t_n_display_name,
   CASE WHEN contr.honor_type_id IS NOT NULL THEN s_email.email ELSE tribute.$t_n_email END as t_n_email, 
  CASE WHEN contr.honor_type_id IS NOT NULL THEN s_addr.street_address ELSE   tribute.$t_n_street_address  END as t_n_street_address ,
  CASE WHEN contr.honor_type_id IS NOT NULL THEN s_addr.city ELSE tribute.$t_n_city END as t_n_city,
  CASE WHEN contr.honor_type_id IS NOT NULL THEN s_state.name ELSE t_n_state.name END  as t_n_state_name, 
  CASE WHEN contr.honor_type_id IS NOT NULL THEN s_country.name ELSE t_n_country.name END as t_n_country_name, 
  CASE WHEN contr.honor_type_id IS NOT NULL THEN s_addr.postal_code ELSE tribute.$t_n_postal_code END as t_n_postal_code   ";
    }
    }
   ...
   if( $tmpEntitlement->isRunningCiviCRM_4_5() ){
     ...
     $tmp_from =  "civicrm_contribution contr 
  	...
  	$tmp_from =  "civicrm_contribution contr 
  			...
  			LEFT JOIN civicrm_state_province t_n_state ON t_n_state.id = $t_n_state_province_id
  			LEFT JOIN civicrm_country t_n_country ON t_n_country.id = $t_n_country_id
  			LEFT JOIN civicrm_contact con2 ON contr.honor_contact_id = con2.id 
  			LEFT JOIN (SELECT ov.value, ov.label as honor_type_label FROM civicrm_option_value ov 
  			JOIN civicrm_option_group og ON ov.option_group_id = og.id AND og.name = 'honor_type' ) as honor_type 
  			 ON honor_type.value  = contr.honor_type_id
  			 LEFT JOIN civicrm_email s_email ON s_email.contact_id = con2.id AND s_email.is_primary =1 
  			  LEFT JOIN civicrm_address s_addr ON s_addr.contact_id = con2.id AND s_addr.is_primary = 1
  			 LEFT JOIN civicrm_state_province s_state ON s_state.id = s_addr.state_province_id
  			 LEFT JOIN civicrm_country s_country ON s_country.id = s_addr.country_id ";
...
$result = civicrm_api('CustomField', 'getsingle', $params);
 ...
 require_once('utils/Entitlement.php');
   ...
   // Either its an "in honor of" using the old Core CiviCRM field, or is a tribute using the newer custom data.    
       $clauses[] = "( contr.honor_contact_id IS NOT NULL OR tribute.$is_tribute_boolean = 1 OR length(tribute.$t_type) > 0 ) "; 
    }
