...
class CRM_Contact_Form_Search_Custom_FinancialAging extends CRM_Contact_Form_Search_Custom_Base implements CRM_Contact_Form_Search_Interface {
...
/***********************************************************************************************/
	...
	// $contrib_type_choices = array( ''   => ' - select contribution type - ' ); 
	...
	$contrib_type_sql = "";
		...
		/*$contrib_type_sql = "Select ft.id, ft.name, fa.accounting_code from civicrm_financial_type ft 
		LEFT JOIN civicrm_entity_financial_account efa ON ft.id = efa.entity_id AND efa.entity_table = 'civicrm_financial_type'
        	LEFT JOIN civicrm_financial_account fa ON efa.financial_account_id = fa.id 
		where ft.is_active = 1 order by ft.name";
		*/
...
/*
	 $tmp_contrib_type_select =  $form->add  ('select', 'contrib_type', ts('Contribution Type'),
                     $contrib_type_choices,
                     false);
                     
         $tmp_contrib_type_select->setMultiple(true); 
  */       
     ...
     if( $entitlement->isRunningCiviCRM_4_5()){ 
      ...
      /*
         $tmp_accounting_code_select =  $form->add  ('select', 'accounting_code', ts('Accounting Code'),
                     $accounting_code_choices,
                     false);
                     
         $tmp_accounting_code_select->setMultiple(true); 
      */  
        ...
        $layout_choice = $this->_formValues['layout_choice'] ;
     	 if ( $onlyIDs ) {
			...
			if( $entitlement->isRunningCiviCRM_4_3()){
				//$groupby = "Group By currency, fa.accounting_code";
				$groupby = "Group By currency";
			}else{
    		
    				$groupby = "Group By currency, ct.accounting_code";
    			}
