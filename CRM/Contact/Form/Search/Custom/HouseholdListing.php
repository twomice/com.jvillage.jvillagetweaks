...
class CRM_Contact_Form_Search_Custom_HouseholdListing extends CRM_Contact_Form_Search_Custom_Base implements CRM_Contact_Form_Search_Interface {
  ...
  function __construct(&$formValues) {
    ...
    $tmp_columns =  array(
      ...
      //ts('Children\'s ages') => 'children_age',
      // ts('Adult B ID') => 'spouse_b_id',
  ...
  function buildForm(&$form) {
    ...
    $form->add('select', 'group_of_contact', ts('Contact is in the group'), $group_ids, FALSE,
      array_merge(array('id' => 'group_of_contact'), $select2style));
    ...
    $form->add('select', 'membership_type_of_contact', ts('Contact has the membership of type'), $mem_ids, FALSE,
      array_merge(array('id' => 'membership_type_of_contact'), $select2style));
    ...
    $form->add('select', 'membership_org_of_contact', ts('Contact has Membership In'), $org_ids, FALSE,
      array_merge(array('id' => 'membership_org_of_contact'), $select2style));
    ...
    $form->add('select', 'membership_primary_choice', ts('Who should be Adult A?'), $primary_mem_options, false);
...
/*
    ...
    $tmp_select = $form->add('select', 'group_of_individual', ts('Individual is in the group'), $group_ids, false);
    $tmp_select->setMultiple(true);

    $mem_ids = $searchTools->getMembershipsforSelectList();
    $tmp_mem_select = $form->add('select', 'membership_type_of_contact', ts('Contact has the membership of type'), $mem_ids, false);
