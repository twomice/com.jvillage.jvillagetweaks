 ...
 */
class CRM_Financial_BAO_ExportFormat_IIFJOURNALACCTNAME extends CRM_Financial_BAO_ExportFormat {
  ...
  function generateExportQuery($batchId) {
    ...
    $sql = "SELECT
      ...
      ft.trxn_date,
      fi.transaction_date as contrib_date, 
      ...
      contact_from.last_name AS contact_from_last_name,
       concat ( contact_to.id , '-', contact_to.display_name) as contact_to_for_memo, 
      ...
      while ($dao->fetch()) {
        ...
        $journalEntries[$dao->financial_trxn_id] = array(
          'to_account' => array(
            ...
            'contrib_date' => $this->format($dao->contrib_date, 'date'), 
            ...
            'description' => $this->format($dao->contact_to_for_memo), 
          ),
          'splits' => array(),
        );
        ...
        if (empty($dao->from_account_id)) {
          ...
          while ($itemDAO->fetch()) {
            ...
            $journalEntries[$dao->financial_trxn_id]['splits'][$itemDAO->financial_item_id] = array(
              ...
              'contrib_date' => $this->format($itemDAO->contrib_date, 'date'),
  ...
  function getHookedTemplateFileName() {
    return 'CRM/Financial/ExportFormat/IIFJOURNALACCTNAME.tpl';
  }
